...
## 模组与进化系统 v3（进行中，里程碑 2/5 已完成）

`DESIGN.md` 已从「13 个平铺强化」的 v2 重写为「开局三选一模组 + 每个强化只有基础/
进化两阶段」的 v3。实施按 5 个里程碑推进，见对应的实施计划。**当前完成到里程碑 2**
（框架 + 通用强化池 + 飞刃模组）；掠影/蓄势各 3 个专属强化排在里程碑 3–4，地图新约束/
Boss 输出窗口/HUD·结算改版排在里程碑 5。
...
- 4 个通用强化（疾风/利刃/韧体/精算）+ 8 条进化，加上飞刃模组的 3 个专属强化
  （贯刃/回旋/刃印）+ 6 条进化，全部按 DESIGN.md §3/§6 的数值实现（细节见下面的
  「里程碑 2」小节）。掠影/蓄势各 3 个专属强化**还没实现**——`UpgradeId` 联合类型
  （`src/i18n/strings.ts`）会在里程碑 3–4 逐个扩展。
- 奖励/商店生成规则重写（新文件 `src/game/rewards.ts`）：战斗房 3 选 1（2 模组专属
  ...
  防无效选项 6 条规则（DESIGN.md §2.4）在这个文件里集中实现。**注意**：选飞刃模组
  时商店/奖励里的专属位已经能正常抽到贯刃/回旋/刃印；选掠影或蓄势模组时专属强化池
  仍是空的，会随 M3–M4 陆续补齐。
- 伤害标签：`World.damageEnemy(e, damage, tag)` 的 `DamageTag` 参数
  （`MELEE|BLADE|DASH|CHARGE|EXPLOSION|AFTEREFFECT`）从 M1 打地基到 M2 第一次被
  真正消费——刃印只认 `BLADE`（叠层）/`MELEE`（消耗），爆印的自动引爆用
  `EXPLOSION` 标签递归调用 `damageEnemy` 自己，天然满足「爆炸不能再触发爆炸」的
  递归禁令。`CHARGE` 标签留给 M4 的震荡打断。
...
强化的贯刃/回旋/刃印全部机制（`tests/bladeModule.test.ts`）**、战斗循环（三模组
各跑一遍 fuzz，另加一条飞刃满强化+环身进化在真实 Boss 房的报价一致性 fuzz）、走表
规则、i18n、启动冒烟测试（含模组选择页流程）。
